...
export PORTAL_SKIN="${portal.skin}"
...
# additional settings
EXTRA_OPTS="-Dserver.url.base=$SERVER_URL -Dserver.ip=$SERVER_IP -Damq.port=$AMQ_PORT -Damq.stomp.port=$AMQ_STOMP_PORT -Dsmtp.host=$SMTP_HOST -Dadmin.email=$ADMIN_EMAIL -Dredbox.version=$REDBOX_VERSION -Dportal.skin=$PORTAL_SKIN"
